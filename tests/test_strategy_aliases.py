"""Branded contest naming queue (AVAX_STRATEGY_0X aliases): registration, bit-for-bit
...
def test_queue_is_registered_and_resolves():
    ...
    assert "AVAX_STRATEGY_02" in avail


def test_alias_target_mapping():
    assert registry.alias_target("AVAX_STRATEGY_02") == "momentum_voltarget"
    assert registry.alias_target("AVAX_STRATEGY_01") == "momentum_adaptive"
...
def test_alias_live_decision_equals_target():
    ...
    a = registry.get("AVAX_STRATEGY_02")
...
def test_alias_selectable_via_strategy_select(tmp_path, monkeypatch):
    ...
    assert strategy_select.save("avax_strategy_02") == "AVAX_STRATEGY_02"
    assert strategy_select.load("momentum_adaptive") == "AVAX_STRATEGY_02"
...
def test_env_strategy_name_selects_alias_on_sim(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(ra.settings, "strategy_name", "AVAX_STRATEGY_02", raising=False)
    # env STRATEGY_NAME is the operator default for BOTH tracks (explicit sign-off)
    assert ra._resolve_strategy_name("sim") == "AVAX_STRATEGY_02"
    assert ra._resolve_strategy_name("live") == "AVAX_STRATEGY_02"
